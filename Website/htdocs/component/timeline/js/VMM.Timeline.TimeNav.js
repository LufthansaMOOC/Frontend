...
define([
    ...
    VMM.master_config.language = VMM.Language;

    if(typeof VMM != 'undefined' && typeof VMM.TimeNav == 'undefined') {

      VMM.TimeNav = function(parent, content_width, content_height) {
        var w = content_width;
        var h = content_height;
        var timeline_id   = "#timelinejs";
        var config = {
          embed:          false,
          events: {
    ...
    function build() {
      ...
      $toolbar					= VMM.appendAndGetElement(layout, "<div>", "mb-toolbar");
    ...
    function buildMarkers() {
      ...
      for(i = 0; i < data.length; i++) {
        ...
        if (data[i].titleEl) {
          _marker_content.append(data[i].titleEl);
        } else if (has_title) {
